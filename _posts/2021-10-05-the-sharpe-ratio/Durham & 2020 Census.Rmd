...
model_2019 <- glm(Party_2019 ~pct_hispanic + pct_white + pct_black + HH_Vacant,family=binomial(link='logit'),data=centroids)
...
pred <- predict(model_2019, newdata = test, type = "response")
...
exp(cbind(OR = coef(model_2019), confint(model_2019)))
...
newdata1 <- with(centroids, data.frame(#pct_hispanic = mean(pct_hispanic,na.rm=T),
...
newdata1$rankP <- predict(model_2021, newdata = newdata1, type = "response")
newdata1
...
predicted <- predict(model_2021, test, type="response") 
