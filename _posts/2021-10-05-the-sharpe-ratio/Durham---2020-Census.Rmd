...
This leads me to the following question: what is the probability of a North Carolian by race to be represented by a democrat or republican? How drastically does this change by race? Finally, how do these probabilities change, if at all, by the new congressional districts proposed. 



### Race by Representative

Average US House republican district in North Carolina with the old lines was actually less more white compared to the new lines proposed. May be just a natural reflection of white population declining in certain areas (see above map). Similarly for Hispanics, those percentages look nearly identical from the old lines to the new lines. 

More interestingly, the average black population increased for democrats by 1.6 percentage points. Could imply the demographic shifts and the drawing of new lines may result in higher likelihood for black North Carolinians to be drawn into a democratic district? Not entirely sold yet. 
...
model_2019_blocks <- glm(PARTY_2019_var ~PCT_HSP + PCT_WHT + PCT_BLC ,family=binomial(link='logit'),data=train)
...
model_2021 <- glm(PARTY_2021 ~PCT_HSP + PCT_WHT + PCT_BLC ,family=binomial(link='logit'),data=train)
...
ggplot(test, aes(x=PCT_BLC, y=PARTY_2019_var)) + geom_point(fill='red') + 
...
ggplot(test, aes(x=PCT_WHT, y=PARTY_2019_var)) + geom_point(fill='red') + 
...
ggplot(test, aes(x=PCT_HSP, y=PARTY_2019_var)) + geom_point(fill='red') + 
...
Predictive fit of model using 2019 congressional districts, this tells us how accurate the supervised regression model performs. For 2019, the machine learning model was able to accurate predict at this rate: 
...
Whereas the 2021 model was able to increase it to: 
...
Finally, the odds ratios below show that 2021 lines are more fair to black North Carolinians. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
boxLabels = c("Hispanic", "White", "Black")

##2019


df <- data.frame(lables=boxLabels, boxOdds = c(1.0014089 , 
0.9951015 , 1.0172783), boxCILow = c(1.0007630 , 0.9948121 ,1.0168304), 
                        boxCIHigh = c(1.0020531, 0.9953909, 1.0177270))
...
ggplot(data = df,
  ...
  #                    labels = (0.5 * (1:10))) +
  labs(x = "Odds Ratio",
       y = "") +theme_minimal() + ggtitle("2019 Model Odds Ratios")


##2021


df <- data.frame(lables=boxLabels, boxOdds = c(1.0003090  , 
0.9960549  , 1.0172424 ), boxCILow = c(0.9995740  , 0.9957221  ,1.0167774 ), 
                        boxCIHigh = c(1.0010402, 0.9963878, 1.0177078))
...
ggplot(data = df,
  ...
  #                    labels = (0.5 * (1:10))) +
  labs(x = "Odds Ratio",
       y = "") +theme_minimal() + ggtitle("2021 Model Odds Ratios")
...
I used to work in the North Carolina General Assembly for division called the Program Evaluation Division, a state level GAO attempting to make government services more efficient and effective. After I left in 2020, the entire shop was eliminated after being used as a political football by schmuck Republican leaders like Phil Berger and Tim Moore. Going into this blog, I didn't exactly expect to see evidence of black and brown citizens being walled into a different political district. The results from this are actually encouraging, perhaps? If I'm allowed to be encouraged by anything in the post COVID-era? 

Ways to improve:

- maybe binning the continuous variables would show more definitive results, or quartiles 
- more explanatory variables such as HH size and Income, can be taken from ACS data and exported as a centroid, tag, same process as used in above
